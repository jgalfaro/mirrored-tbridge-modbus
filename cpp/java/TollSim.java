...
public class TollSim extends Device {
     ...
     */
    public void run(SingleThreadedServer server) {
     ...
     * draw screen
     */
    public void drawScreen(SingleThreadedServer server) {

        try{
            String status = "";
            status += "---------------\n";
            if(this.spi.getRegister(STATUS_NB_COINS).getValue()==0){
                status += "Coins inside   : " + this.spi.getRegister(STATUS_NB_COINS).getValue() + "\n";
            }else{
                status += "Coins inside   : " + (this.spi.getRegister(STATUS_NB_COINS).getValue()+1) + "\n";
            }
            ...
            status += "---------------\n\n\n";
            status += "ACTIONS: URI/?addcoin --> ADD A NEW COIN\n";
    ...
    public void setCoin(int coinValue) {
        System.out.println("New coin inserted");// + coinValue);
